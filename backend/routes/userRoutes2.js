...
import { isAuth, isStaff, generateToken, baseUrl, mailgun } from '../utils.js';
...
userRouter2.delete(
  ...
  expressAsyncHandler(async (req, res) => {
    const user = await User.findById(req.params.id);
    if (user) {
      if (user.email === 'StaffSavore@example.com') {
        res.status(400).send({ message: 'Can Not Delete Staff User' });
        return;
      }
